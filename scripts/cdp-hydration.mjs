...
const ROUTES = routesFlag
  ? routesFlag
      ...
      .map((r) => r.trim())
      .filter(Boolean)
  : // Дефолт обязан покрывать ВСЕ живые маршруты: маршрута, которого прибор
    // не открывает, для него не существует — он молча рапортует «чисто по
    // сайту», проверив половину. `/collections/optical` появился этапом 12а
    // и полтора этапа не проверялся ничем. Каждый новый маршрут витрины
    // дописывается СЮДА тем же коммитом, что и сам маршрут.
    ["/", "/collections/optical", "/privacy"];
