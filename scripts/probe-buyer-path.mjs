...
async function main() {
  ...
  try {
    ...
    if (MODE === "main") {
      ...
      /**
       * ⚠️ ДВА ПРЕДОХРАНИТЕЛЯ, БЕЗ КОТОРЫХ ЭТА ПРОВЕРКА БРАКУЕТ ГОДНОЕ.
       * Первая версия сравнивала ступеньку у шва с ВЫДУМАННЫМ порогом 12
       * и давала два «провала» на исправном коде:
       *  1. на −6 дптр шва НЕТ ВОВСЕ — шторка стоит на правом краю кадра,
       *     и «ступенька 120» была границей КАДРА и фона страницы. Это исход
       *     «СУДИТЬ НЕЧЕМ»: критерий обязан сперва доказать, что явление
       *     попало в окно замера (закон CLAUDE.md);
       *  2. порога у шва не существует в природе — мера только
       *     ОТНОСИТЕЛЬНАЯ: собственная текстура кадра (листва) рвётся
       *     по колонкам сильнее любого артефакта склейки. Сравниваем
       *     ступеньку у шва с максимальным перепадом ВДАЛИ от шва, в том
       *     же кадре и той же полосе высоты.
       * Те же два предохранителя живут в `probe-seam.mjs` (offline-разбор).
       */
      const seam = (frame, st, label) => {
        ...
        const left = st.rect.l * DSF;
        const right = (st.rect.l + st.rect.w) * DSF;
        // Шов у самой кромки кадра — двух сторон не существует.
        if (xc - left < 24 || right - xc < 24)
          return { verdict: "нечем", xc, msg: `шов на кромке кадра (x=${xc.toFixed(0)}, кадр ${left.toFixed(0)}…${right.toFixed(0)})` };
        ...
        /* Фон сравнения — СОБСТВЕННАЯ ТЕКСТУРА КАДРА, и мерить её надо ПО ВСЕМУ
           кадру, а не в одном месте. Первая версия брала одно окно на 40 %
           расстояния до кромки, оно попадало в РАЗМЫТУЮ половину (там колонки
           почти не рвутся: перепад 0,78) — и «фоном» становился гладкий кисель,
           против которого любой честный шов выглядит артефактом. Берём максимум
           по сетке окон с обеих сторон, кроме полосы ±24 px у шва и кромок. */
        let base = 0;
        for (let f = 0.08; f <= 0.92; f += 0.06) {
          const bx = left + (right - left) * f;
          if (Math.abs(bx - xc) < 24 || bx - left < 16 || right - bx < 16) continue;
          const bprof = columnProfile(frame.raw, frame.info, bx, y0, y1, 8);
          for (let k = 1; k < bprof.length; k++)
            base = Math.max(base, Math.abs(bprof[k].mean - bprof[k - 1].mean));
        }
        console.log(`  ${label}: профиль ${prof.map((c) => c.mean.toFixed(0)).join(" ")}`);
        return { verdict: "мерено", maxJump, at, base };
      };
      console.log("\nстык шторки (яркость по колонкам вокруг линии, физ. px):");
      const cases = [
        [results[3].frame, results[3].st, "покой −6"],
        [results[2].frame, results[2].st, "покой −3"],
        ...(mid.frame ? [[mid.frame, mid.st, "в движении"]] : []),
      ];
      let seamJudged = 0;
      for (const [fr, st, label] of cases) {
        const s = seam(fr, st, label);
        if (s.verdict === "нечем") {
          console.log(`  ⚪ ${label}: СУДИТЬ НЕЧЕМ — ${s.msg}`);
          continue;
        }
        seamJudged++;
        console.log(
          `  ◻ ${label}: ступенька у шва ${s.maxJump.toFixed(2)} против собственной ` +
            `текстуры кадра ${s.base.toFixed(2)} (число, НЕ вердикт — см. ниже)`
        );
      }
      /**
       * ⛔️ ВЕРДИКТА ПО ШВУ ЗДЕСЬ НЕТ, И ЭТО ЧЕСТНЕЕ ЛЮБОГО «✅».
       *
       * Критерий выше сравнивает КОЛОНКИ ПО РАЗНЫЕ СТОРОНЫ ШВА, а канон
       * запрещает это прямым текстом: по разные стороны границы лежат разные
       * куски изображения И разные способы его рендера (слева — размытый
       * канвас с потерей насыщенности и халяцией, справа — резкий DOM), поэтому
       * такой замер меряет СЮЖЕТ, а не шов. Признак того, что он в шуме, —
       * налицо: два прогона дали противоположные вердикты на одном коде
       * (приёмщик 15,77 против 15,92 — проход; этот прогон 16,26 против
       * 11,13 — провал), потому что кадр «в движении» каждый раз ловится
       * в другом положении и сравнивается с другой листвой.
       *
       * Печатать числа — полезно (регрессия в разы будет видна глазом).
       * Выносить вердикт — нельзя: прибор, отвечающий монеткой, хуже
       * отсутствующего. Правильный метод — не яркость, а ГЕОМЕТРИЯ:
       * положение обрезки канваса против положения линии-ручки на одном и том
       * же кадре (тот же урок, каким приёмщик заменил корреляцию яркостей
       * прямым замером рамок). Это остаток этапа 6, зона `fizik`.
       */
      console.log(
        "  ⚠️ вердикт по шву НЕ ВЫНОСИТСЯ: критерий сравнивает две стороны границы\n" +
          "     (канон это запрещает — меряется сюжет, а не шов). Нужен геометрический\n" +
          "     замер: обрезка канваса против линии-ручки. Числа выше — справочные."
      );
      if (!seamJudged)
        console.log("  ⚪ ни одна ступень не дала шва внутри кадра — двигай ступени внутрь");
