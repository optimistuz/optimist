 ...
 *
 * 2. **Роутер `next/link` в бандле.** Сайт живёт на `<a>` и `AnimatedLink`;
 *    первый же импорт `next/link` в компонент главной стоил **9 КБ**
 *    (196 → 205 КБ, мимо потолка 197) — этап 11 поймал это `next build`-ом,
 *    а не глазами. Когда `next-view-transitions` войдёт осознанно (этап 12),
 *    сюда добавится исключение — но добавится ЯВНО, решением, а не случайно.
 *
