...
/** Путь сечения минус-линзы: толстые края, тонкая талия по центру.
    Каждая поверхность — ОДНА гладкая квадратичная кривая (плавная ложбина
    сверху, плавный гребень снизу). Контрольная точка подобрана так, чтобы
    середина кривой пришлась ровно в ±TC от оси: для квадратичной Безье
    y(середина) = ¼·y0 + ½·yctrl + ¼·y2, отсюда yctrl = CY ∓ (2·TC − e).
    Раньше тут стыковались две кривые с встречными вертикальными касательными
    и в центре росли острые шипы — линза читалась как «непонятно что». */
const pathFor = (thin: number) => {
  const e = edgeFor(thin);
  const hi = (CY - e).toFixed(1); // верхняя кромка края
  const lo = (CY + e).toFixed(1); // нижняя кромка края
  const topCtrl = (CY - 2 * TC + e).toFixed(1); // ложбина к −TC в центре
  const botCtrl = (CY + 2 * TC - e).toFixed(1); // гребень к +TC в центре
  return [
    `M ${X_LEFT} ${hi}`,
    `Q ${X_MID} ${topCtrl} ${X_RIGHT} ${hi}`,
    `L ${X_RIGHT} ${lo}`,
    `Q ${X_MID} ${botCtrl} ${X_LEFT} ${lo}`,
    "Z",
  ].join(" ");
};
