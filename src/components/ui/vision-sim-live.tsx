...
/**
 * ЗАПЕЧЁННЫЕ СЛОИ СВЕТОВ по кадрам (шаг 7 — халяция; сам слой ниже).
 *
 * ⚠️ ПУТИ ЛИТЕРАЛАМИ, А НЕ `/photos/${slot}-highlights.jpg`. Собранная
 * строкой ссылка обещала бы файл, которого никто не проверял: греп по имени
 * ассета не нашёл бы НИ ОДНОГО упоминания, а на месте слоя поехал бы 404.
 * Здесь же слой существует ровно для тех кадров, для которых он запечён,
 * и включить его для чужого кадра нельзя в принципе.
 *
 * ⚠️ КАРТА ЖИВЁТ ЗДЕСЬ, А НЕ В ПРОПАХ ОБОЛОЧКИ, — по закону разреза: слой
 * светов не нужен HTML ни кадром, ни высотой, ни ARIA, а проп через оболочку
 * стоил критическому пути «/» +11 Б (замер `next build`).
 *
 * Кадр попадает в карту, только если в нём ЕСТЬ ЧЕМУ разливаться (небо
 * сквозь листву, контровой свет). У страницы книги в помещении светов нет:
 * слой был бы честным нулём и стоил бы лишнего запроса и декода.
 * Происхождение ассетов и рецепт запекания — `src/content/photos.ts`.
 */
const HIGHLIGHTS: Partial<Record<PhotoSlot, string>> = {
  street: "/photos/street-highlights.jpg",
};
...
export function VisionSimLive({
  ...
  /** Запечённый слой светов этого кадра — или его нет вовсе (см. HIGHLIGHTS). */
  const halationSrc = HIGHLIGHTS[slot];

  /**
   * ВТОРОЙ РУБЕЖ ПОЛА ГРУППЫ: слой светов не выходит, пока базовый кадр не
   * нарисован. Первый рубеж — непрозрачный `bg-paper` на контейнере
   * (`vision-sim.tsx`), и его достаточно против тёмной плиты; этот закрывает
   * то, что полом не лечится:
   *  · слот фото ПУСТ — на его месте `Placeholder`, и «разлив светов» шёл бы
   *    поверх фотографии, которой нет (нашёл `fizik`);
   *  · кадр ещё не доехал по сети — свет обязан появляться ВМЕСТЕ с тем, от
   *    чего он исходит, а не раньше него.
   * Признак тот же, каким `vision-blur.tsx` ищет текстуру: `<img>` кадра,
   * кроме самого слоя светов.
   */
  const [baseDrawn, setBaseDrawn] = useState(false);
  useEffect(() => {
    if (!halationSrc) return;
    const img = hostRef.current?.querySelector<HTMLImageElement>(
      "img:not([data-halation])"
    );
    if (!img) return; // слот пуст — слоя светов не будет вовсе
    if (img.complete && img.naturalWidth > 0) {
      setBaseDrawn(true);
      return;
    }
    const onLoad = () => setBaseDrawn(true);
    img.addEventListener("load", onLoad, { once: true });
    return () => img.removeEventListener("load", onLoad);
  }, [halationSrc, hostRef]);
  const halation = baseDrawn ? halationSrc : undefined;
  ...
  return (
    <>
      ...
      {/* ХАЛЯЦИЯ СВЕТОВ (шаг 7) — физика близорукости, а не украшение.

          Расфокусированный глаз не просто мылит кадр: яркие источники
          РАЗЛИВАЮТСЯ за свои границы, и чем сильнее минус, тем шире разлив.
          Именно это отличает честную имитацию от «фотографии под матовым
          стеклом» — размытие само по себе делает кадр темнее и мертвее.

          ⚠️ Слой ЗАПЕЧЁН (`bake-blur.mjs highlights` из кадра секции: света
          выше порога сохранены, остальное — чёрное, поверх разлив σ = 24 —
          столько же, сколько радиус расфокуса на −6 дптр; вывод числа —
          в `photos.ts`). Живого фильтра здесь нет и быть не может: закон
          движка №2 — тяжёлый blur не живёт в CSS. Чёрное под `screen`
          прозрачно, поэтому подложка именно чёрная, а не альфа-канал
          (JPEG вместо PNG — 21 КБ вместо сотен).

          ⚠️ `data-halation` — НЕ разметка для стилей, а метка ИСКЛЮЧЕНИЯ.
          Текстуру Kawase берёт из уже отрисованного `<img>` внутри кадра
          (`vision-blur.tsx`), находя его `querySelector`-ом. Этот слой —
          тоже `<img>` внутри кадра. Пока фото слота на месте, первым в
          документе лежит оно, и всё сходится само; но если слот ПУСТ, на его
          месте `Placeholder` (div, не img) — и поиск дотянулся бы до светов.
          Тогда «без коррекции» показывало бы размытую почти-чёрную маску
          вместо честного отказа в CSS-фолбэк (ветку `if (!img) fail()` нашёл
          `fizik` — ровно этот класс молчаливой поломки).

          ⚠️ ДВИЖЕНИЯ В СЛОЕ НЕТ — меняется только прозрачность, и только от
          положения шторки. Это отклик на жест, а не бесконечная анимация:
          бюджет вечного движения (ровно две) не тронут.

          ⚠️ ПРОЗРАЧНОСТЬ = СИЛА ДЕФЕКТА, без множителей: амплитуду разлива
          задаёт запечённый слой (порог и радиус), см. `photos.ts`. В нуле
          диоптрий слой обязан быть НЕВИДИМ В ТОЧНОСТИ — иначе рушится закон
          «при нуле левая половина резка в точности как правая»; нулевая
          прозрачность не рисуется вовсе, и это подтверждено пиксельно
          (`probe-halation.mjs`: Δ = 0 по всему кадру).

          Клип и маска глубины — те же, что у размытия: халяция живёт только
          на стороне «без коррекции» и гаснет там же, где гаснет расфокус.

          ⚠️ `scale-105` ТОЛЬКО В ФОЛБЭК-ВЕТКЕ, и это не косметика. Размываемый
          слой фолбэка растянут на 105 % (иначе CSS-blur обнажает прозрачную
          кромку), а GL-канвас натягивает текстуру cover-обрезкой один к одному.
          Слой светов обязан совпадать с тем, поверх чего лежит: без этого
          в фолбэке свет уезжал от своего источника на 29,6 CSS px у кромки
          кадра — ВДВОЕ дальше, чем сам источник размазан радиусом 15,4 px
          (замер `fizik` корреляцией полос: 20 px у кромки, 3 px у центра —
          градиент ровно как у пятипроцентного масштаба). Ставить `scale-105`
          безусловно НЕЛЬЗЯ: в GL-ветке это разъехало бы совпадающее.
          Трансформ — на ВНУТРЕННЕМ узле: на самом слое он сменил бы группу
          смешивания. */}
      {halation && (
        <motion.div
          aria-hidden
        ...
        >
          <div className={cn("absolute inset-0", !glLive && "scale-105")}>
            <Image
              src={halation}
              alt=""
              data-halation=""
