  ...
  return (
    <div className={className}>
      {/* Сцена-шторка: резкая база всегда в HTML, размываемый слой — за живой частью.
          ...
          ⚠️ `isolate` — не украшение: слой халяции (шаг 7) кладётся через
          `mix-blend-mode: screen`, а blend смешивается ВНУТРИ stacking context.
          Без явной изоляции группой становилась бы обёртка `RevealItem` —
          на время `imageReveal` у неё opacity < 1, и она превращается
          в stacking context. Здесь граница объявлена руками.
          ⚠️ Замер `fizik`: в ПОКОЕ изоляция не меняет ни одного пикселя
          (медиана и p99 равны 0) — она нужна ровно на время появления.

          ⚠️ `bg-paper` — НЕПРОЗРАЧНЫЙ ПОЛ ГРУППЫ СМЕШИВАНИЯ, и это не
          декорация фона. `screen` смешивается с backdrop-ом группы, а по
          формуле композитинга при полностью прозрачном backdrop результат
          равен ИСХОДНИКУ — то есть почти чёрной маске светов. Пока базовое
          фото не нарисовано (`Photo` держит `opacity-0` до `load`, плюс фейд
          500 мс), пол группы был прозрачен, и слой светов ложился ТЁМНОЙ
          ПЛИТОЙ на левую половину кадра: замер `fizik` на задержанном фото —
          128,8 против 249,0 на резкой стороне, окно 6,7…11,2 с. На LTE это
          не экзотика: слой весит 9,5 КБ, фото — 533 КБ, оба ленивые
          и стартуют с одного порога, поэтому слой приходит первым ВСЕГДА.
          Снятие `isolate` починкой НЕ является (проверено: с `opacity: 0.99`
          на предке плита возвращается, −232). Лечит именно пол. */}
      <div
        ref={containerRef}
        className="relative isolate aspect-[4/3] cursor-ew-resize select-none overflow-hidden rounded-2xl bg-paper sm:aspect-[16/9]"
      ...
      >
        ...
        <div
          ...
          /* ⚠️ `pan-y`, А НЕ `none` — ТАЧ-АНАЛОГ ЗАКОНА О КОЛЕСЕ. Ручка это
             полоса ВО ВСЮ ВЫСОТУ кадра (`inset-y-0`) плюс кнопка 40×40 —
             самое приглашающее место для пальца. С `touch-action: none` палец,
             опустившийся на кнопку и поехавший ВНИЗ, не получал ничего:
             страница не скроллилась (панорамирование запрещено), шторка не
             двигалась (она следит только за X). Ровно та кража скролла,
             которую канон запрещает колесу. `none` здесь не покупал ничего:
             перетаскивание слушает ОБОЛОЧКА, а у неё `pan-y` (см. выше), то
             есть горизонталь и так достаётся JS; `touch-action` пересекается
             с предками, поэтому `pan-y` ∩ `pan-y` = `pan-y`.
             Нашёл `dirizher` чтением кода — там, где синтетический прибор
             приёмщика честно ответил «судить нечем» (он не умеет скроллить
             страницу тачем даже ВНЕ кадра, то есть мерил бы себя).
             ⛔️ Пальцем на живом телефоне НЕ подтверждено — это пункт вечера
             с A54 (остаток этапа 6). Правка сделана вперёд, потому что цена
             ошибки несимметрична: `none → pan-y` только ДОБАВЛЯЕТ вертикальное
             панорамирование и ничего не отнимает (`pan-y` так же запрещает
             горизонталь, пинч и двойной тап), а нетронутая строка — это
             застрявший палец у каждого, кто взялся за ручку и повёл вниз. */
          style={{ left: `${startPos}%`, touchAction: "pan-y" }}
        >
