...
/**
 * ⚠️ У ХАЛЯЦИИ (шаг 7) СВОЕЙ КОНСТАНТЫ СИЛЫ НЕТ — И ЭТО РЕШЕНИЕ, А НЕ
 * ЗАБЫВЧИВОСТЬ. Прозрачность слоя светов равна силе дефекта один к одному;
 * амплитуду задаёт САМ ЗАПЕЧЁННЫЙ СЛОЙ (порог светов и радиус разлива,
 * см. `src/content/photos.ts` → «производные ассеты»), а не второй
 * множитель поверх него.
 *
 * Множитель здесь был (0,55) и опирался на утверждение «на полной
 * прозрачности света выбьют небо в белое». Утверждение ОПРОВЕРГНУТО
 * замером (`scripts/probe-halation.mjs`): пикселей ярче 252 при слое
 * и без слоя ровно одинаково — 0,179 %, ни один свет не выбит. Зато при
 * 0,55 разлив не читался вовсе (пик +13,8 из 255 против +31,1 при
 * единице), то есть множитель молча гасил эффект, ради которого шаг сделан.
 * Свободный параметр, ничего не удерживающий, — просто место для ошибки.
 */
...
export type VisionSimShared = {
  ...
  depthMask?: boolean;
  nudge?: boolean;
  /* ⚠️ ХАЛЯЦИИ (шаг 7) ЗДЕСЬ НЕТ, И ЭТО ПО ЗАКОНУ РАЗРЕЗА. Слой светов —
     машинерия, а не разметка: в HTML он не нужен ни кадром, ни высотой, ни
     ARIA. Проп через оболочку стоил критическому пути «/» +11 Б (замер
     `next build`), и байты эти платил бы каркас за чужой слой. Есть ли у кадра
     что разливать — свойство КАДРА, поэтому карта слоёв живёт в ленивом
     `vision-sim-live.tsx` рядом с самим слоем, а происхождение ассета —
