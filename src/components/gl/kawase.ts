...
import { allocRatio, nextAlloc } from "@/lib/kawase-alloc";
...
export function createKawase(
   ...
   * контейнера.
   *
   * ⚠️ И САМА ФУНКЦИЯ, И ПРАВИЛО РОСТА ЖИВУТ В `lib/kawase-alloc.ts` —
   * чтобы их можно было СПРОСИТЬ из юнит-теста, а не переписать там
   * заново. Расхождение размещённого размера с текущим даёт значения
   * нормировки, которых нет в наборе «плотность × клапан», и §6-11
   * проверяется именно на них (перебор ПАР раскладок).
   */
  const ratio = () => allocRatio(width, cssWidth);
  ...
  function resize(w: number, h: number, cw: number) {
    ...
    cssWidth = cw;
    // ⚠️ ПЕРЕСОБИРАЕМ ТОЛЬКО НА РОСТ — правило и его цена описаны в
    // `lib/kawase-alloc.ts`; здесь оно ВЫЗЫВАЕТСЯ, а не повторяется.
    ...
    const next = nextAlloc({ width, height, allocated: targets.length > 0 }, w, h);
    if (!next.rebuild) return;
    width = next.width;
    height = next.height;
    dropTargets();
