...
export function VisionBlur({
  ...
  const setup = useCallback(
    ({ renderer, gl }: GLSetupArgs): GLScene => {
      ...
      const attach = () => {
        /**
         * ⚠️ `:not([data-halation])` — слой светов шага 7 тоже `<img>` внутри
         * кадра. Пока фото слота на месте, оно первое в документе и поиск
         * находит его; но при ПУСТОМ слоте на месте фото стоит `Placeholder`
         * (div), и простой `querySelector("img")` дотянулся бы до светов —
         * текстурой стала бы почти чёрная маска, а честный отказ ниже
         * не сработал бы вовсе.
         */
        const img =
          hostRef.current?.querySelector<HTMLImageElement>(
